...
// func mappedDomainsFromList(vs []interface{}, resourceID string) ([]MappedDomain, error) {
// 	result := make([]MappedDomain, 0, len(vs))
...
// 		t, err := mappedDomainFromMap(attr, resourceID)
// 		if err != nil {
// 			return nil, err
// 		}
...
// func mappedDomainFromMap(attr map[string]interface{}, resourceID string) (*MappedDomain, error) {
// 	if _, ok := attr["mapped_domain"]; !ok {
// 		return nil, fmt.Errorf("%s: invalid mapped_domain attributes: mapped_domain missing", resourceID)
// 	}

// 	if _, ok := attr["name"]; !ok {
// 		return nil, fmt.Errorf("%s: invalid mapped_domain attributes: name missing", resourceID)
// 	}

// 	var enterpriseDNS bool
// 	var err error

// 	if v, ok := attr["enterprise_dns"].(bool); ok {
// 		enterpriseDNS = v
// 	}

// 	if v, ok := attr["enterprise_dns"].(string); ok {
// 		if enterpriseDNS, err = strconv.ParseBool(v); err != nil {
// 			return nil, fmt.Errorf("%s: invalid mapped_domain attribute: invalid value for enterprise_dns: %s", resourceID, v)
// 		}
// 	}

// 	t := &MappedDomain{
// 		EnterpriseDNS: enterpriseDNS,
// 		MappedDomain:  attr["mapped_domain"].(string),
